// Canvas line chart with multi-lap series, secondary axis, shared cursor, touch pinch-zoom and pan.
...
export class LineChart {
  ...
  constructor(canvas, opts = {}) {
    ...
    this.gesture = null;
    this.raf = 0;
    this.reserveRight = 0; // px kept free at the top-right (panel title chip)
  ...
  resize() {
    ...
    const h = Math.max(50, parent.clientHeight);
    this.w = w; this.h = h;
  ...
  /**
   * @param {object} d
   * d.series: [{x,y,n,color,width,label}], d.series2 (thin, right axis), d.markers: [{x,label,color}]
   * d.xMax, d.xLabel, d.yLabel, d.y2Label, d.fmt(v)=>string, d.fmt2, d.zeroLine, d.yRange, d.y2Range
   */
  setData(d) {
    ...
    this.yLabel = d.yLabel || '';
    this.y2Label = d.y2Label || '';
    ...
    this.yRange = d.yRange || null;
    this.y2Range = d.y2Range || null;
    this.empty = d.empty || '';
    if (this.view) {
      // clamp view
      const [a, b] = this.view;
  ...
  resetView() { this.setView(0, this.xMax); }
  ...
  computeYRange(list, forced) {
    ...
    for (const s of list) {
      for (let i = 0; i < s.n; i++) { const y = s.y[i]; if (y < lo) lo = y; if (y > hi) hi = y; }
    }
  ...
  draw() {
    ...
    const yr = this.computeYRange(this.series, this.yRange);
    ...
    if (yr2) {
      ...
      ctx.textAlign = 'left';
      for (let y = Math.ceil(yr2[0] / ystep2) * ystep2; y <= yr2[1] + 1e-9; y += ystep2) {
        ctx.fillText(formatTick(y, ystep2), r.x + r.w + 6, y2ToPx(y));
      }
    }
    this._drawXAxis(ctx, r, v, colGrid, colText, colAxis);
    ...
    // axis labels
    ctx.save();
    ctx.translate(12, r.y + r.h / 2); ctx.rotate(-Math.PI / 2);
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.font = FONT; ctx.fillStyle = colText;
    ctx.fillText(this.yLabel, 0, 0);
    ctx.restore();
    if (yr2) {
      ctx.save();
      ctx.translate(this.w - 8, r.y + r.h / 2); ctx.rotate(Math.PI / 2);
      ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.font = FONT; ctx.fillStyle = colText;
    ...
    if (this.zeroLine && yr[0] < 0 && yr[1] > 0) {
      ctx.strokeStyle = colAxis; ctx.lineWidth = 1; ctx.beginPath();
      const py = yToPx(0); ctx.moveTo(r.x, py); ctx.lineTo(r.x + r.w, py); ctx.stroke();
    }
    ...
    for (const m of this.markers) {
      ...
      ctx.setLineDash([]);
      if (m.label) {
        ctx.fillStyle = m.color || colMarker; ctx.font = FONT; ctx.textAlign = 'left'; ctx.textBaseline = 'top';
        ctx.fillText(m.label, px + 3, r.y + 3);
      }
    }
    ...
    const drawSeries = (list, toPx, width) => {
      for (const s of list) {
        ...
        ctx.beginPath();
        let started = false;
        // find visible index range with binary search‑ish scan
        let i0 = 0; while (i0 < s.n - 1 && s.x[i0 + 1] < v[0]) i0++;
        const stepPx = r.w / Math.max(1, s.n); // decimation: skip points closer than 0.5px
        let lastPx = -Infinity;
    ...
    if (Number.isFinite(this.cursor) && this.cursor >= v[0] && this.cursor <= v[1]) {
      ...
      ctx.strokeStyle = colCursor; ctx.lineWidth = 1.5;
      ctx.beginPath(); ctx.moveTo(px, r.y); ctx.lineTo(px, r.y + r.h); ctx.stroke();
      // points on series
    ...
    // cursor value labels (top rows) – primary bold, secondary small; wraps and avoids the reserved right area
    if (Number.isFinite(this.cursor)) {
      ...
      if (px >= r.x && px <= r.x + r.w) {
        ctx.font = FONT_BOLD; ctx.fillStyle = colCursor; ctx.textAlign = 'center'; ctx.textBaseline = 'top';
  ...
  _bind() {
    ...
    this._onDown = (e) => {
      ...
      if (this.pointers.size === 1) {
        this.gesture = { type: 'cursor', startX: e.offsetX, startY: e.offsetY, moved: false, t0: performance.now() };
        ...
        this.gesture = { type: 'pinch', startDist: Math.abs(a.x - b.x) || 1, startCenter: (a.x + b.x) / 2, view0: this.getView() };
      }
    };
    this._onMove = (e) => {
      ...
      if (this.gesture.type === 'cursor') {
        ...
        const r = this.plotRect();
        const [v0, v1] = this.gesture.view0;
        const span0 = v1 - v0;
        const scale = this.gesture.startDist / dist;
        let span = Math.min(this.xMax, Math.max(this.xMax * 0.01, span0 * scale));
        // keep the x under the initial center fixed, then pan by center movement
        const xAtStart = v0 + ((this.gesture.startCenter - r.x) / r.w) * span0;
        const f = (this.gesture.startCenter - r.x) / r.w;
        let na = xAtStart - span * f;
        na -= ((center - this.gesture.startCenter) / r.w) * span;
    ...
    this._onWheel = (e) => {
      ...
      if (e.ctrlKey || Math.abs(e.deltaY) > Math.abs(e.deltaX)) {
        ...
        this.zoomBy(factor, this.pxToX(e.offsetX, r, v));
      } else {
  ...
  _pxToXClamped(px) {
    const r = this.plotRect(); const v = this.getView();
    return Math.max(v[0], Math.min(v[1], this.pxToX(px, r, v)));
  }
  _cursorFromPx(px) {
    const x = this._pxToXClamped(px);
    this.cursor = x;
    this.requestDraw();
    if (this.opts.onCursor) this.opts.onCursor(x);
  }
}
...
/** Scatter plot (G‑force): x = lateral, y = longitudinal. */
export class ScatterChart {
  constructor(canvas) {
    this.canvas = canvas; this.ctx = canvas.getContext('2d');
    this.series = []; this.range = 2.5;
    this.dpr = Math.max(1, Math.min(3, window.devicePixelRatio || 1));
    this.ro = new ResizeObserver(() => this.resize());
    this.ro.observe(canvas.parentElement || canvas);
    this.resize();
  ...
  /** series: [{x:Float32Array,y:Float32Array,n,color,label, highlight?:{x,y}}] */
  ...
  draw() {
    ...
    for (let v = -Math.floor(R / step) * step; v <= R + 1e-9; v += step) {
      ...
      ctx.textAlign = 'center'; ctx.textBaseline = 'top'; ctx.fillText(v.toFixed(1), X(v), oy + size + 4);
      ctx.textAlign = 'right'; ctx.textBaseline = 'middle'; ctx.fillText(v.toFixed(1), ox - 4, Y(v));
    }
    ...
    for (let g = 1; g <= R; g++) { ctx.beginPath(); ctx.arc(X(0), Y(0), (g / (2 * R)) * size, 0, Math.PI * 2); ctx.stroke(); }
    ctx.setLineDash([]);
    for (const s of this.series) {
      ctx.fillStyle = s.color;
      for (let i = 0; i < s.n; i++) { ctx.beginPath(); ctx.arc(X(s.x[i]), Y(s.y[i]), 1.8, 0, Math.PI * 2); ctx.fill(); }
    }
    for (const s of this.series) {
      if (s.highlight && Number.isFinite(s.highlight.x)) {
        ctx.fillStyle = s.color; ctx.strokeStyle = '#fff'; ctx.lineWidth = 1.5;
        ctx.beginPath(); ctx.arc(X(s.highlight.x), Y(s.highlight.y), 6, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
      }
    }
    ctx.fillStyle = colText; ctx.textAlign = 'center'; ctx.textBaseline = 'bottom';
    ctx.fillText(this.xLabel || '', ox + size / 2, this.h - 2);
